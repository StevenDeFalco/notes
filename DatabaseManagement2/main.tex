...
A \define{transaction} is a unit of program execution that accesses and possibly updates various data items. To preserve the integrity of data, the database system must ensure:
\begin{itemize}
  \item \bold{Atomic}: either all operations of the transaction are properly reflected in the database or none are
  \item \bold{Consistency}: execution of a transaction in isolation preserves the consistency of the database
  \item \bold{Isolation}: Although multiple transactions may execute concurrently, each transaction must be unaware of other concurrently executing transactions
  \item \bold{Durability}: After a transaction completes successfully, the changes it has made to the database perist, even if there are system failures. 
\end{itemize}
A transaction has many states during its execution: 
\begin{itemize}
  \item \bold{active}---the initial state; the transaction stays in this state while it is executing 
  \item \bold{partially committed}---after the final statement has been executed 
  \item \bold{failed}---after the discovery that normal execution can no longer proceed 
  \item \bold{abored}---after the transaction has been rolled back and the database restored to its state prior to the start of the transaction (restart or kill) 
  \item \bold{committed}---after successful completion
\end{itemize}
The \define{recovery-management} component of a database system implements the support for atomicity and durability. The \define{shadow-database scheme} assumes that only one transaction is active at a time. A pointer called \emph{db\_pointer} always points to the current conssitent copy of the database. All updates are made on the \emph{shadow copy} of the database, and \bold{db\_pointer} is made to point at the updated shadow copy only after the transaction reaches partial commit and all updated pages have been flushed to disk. In case transaction fails, old consistent copy pointed to by \bold{db\_pointer} can be used, and the shadow copy can be deleted. \\ 

Multiple transaction are allowed to run concurrently in the system. The advantages of this are increased processor and disk utilization and reduced average response time. \define{Concurrency control schemes} are mechanisms to achieve isolation; that is, to control the interaction among the concurrent transactions in order to prevent them from destroying the consistency of the database. \\ 

A \define{schedule} is a sequence of instructions that specidfy the chronological order in which instructions of concurrent transactions are executed. A schedule for a set of transactions must consist of all instructions of those transactions and must preserve the order in which the instructions appear in each individual transaction. A transaction that successfully completes its execution will have a commit instructions as the last statement. A transaction that fails to successfully complete its execution will have an abort instructions as the last statement. \\ 

\begin{remark}
  \define{Serial} schedules are safe but not efficient. \define{Concurrent} schedules are efficient but can be unsafe. 
\end{remark}

A schedule is \define{serializable} if it is equivalent to a serial schedule (i.e. a concurrent schedule that can be transformed into an equivalent serial schedule). Instructions will \bold{conflict} if there are writes in any of the instructions. Different forms of schedule equivalence give rise to the notions of: 
\begin{itemize}
  \item Conflict serializability 
  \item View serializability
\end{itemize}

If a schedule $S$ can be transformed into a schedule $S'$ by a series of swaps of non-conflicting instructions, we say that $S$ and $S'$ are \define{conflict equivalent}. We say that a schedule $S$ is \define{conflict serializable} if it is conflict equivalent to a serial schedule. \\ 

\begin{remark}
  For a schedule to be conflict serializable, you must be able to move the order of any instruction which does not include a write. 
\end{remark}

Let $S$ and $S'$ be two schedules with the same set of transactions. $S$ and $S'$ are \define{view equivalent} if the follownig three conditions are met: 
\begin{enumerate}
  \item For each data item $Q$, if transaction $T_i$ reads the initial value of $Q$ in schedule $S$, then transaction $T_i$ must, in schedule $S'$, also read the intial value of $Q$. 
    \begin{itemize}
      \item \bold{First to Read Rule}
      \item Need to know which transaction reads the data item first. 
    \end{itemize}
  \item For each data item $Q$ if transaction $T_i$ executes \bold{read}($Q$) in schedule $S$, and that value was produced by transaction $T_j$ (f any), then transaction $T_i$ must in schedule $S'$ also read the value of $Q$ that was produced by transaction $T_j$. 
    \begin{itemize}
      \item \bold{Producer-Consumer Rule}
      \item Between 2 transaction, we need to know which one writes and which one reads. 
    \end{itemize}
  \item For each data item $Q$, the transaction (if any) that performs the final \bold{write}($Q$) operation in schedule $S$ must perform the final \bold{write}($Q$) operation in schedule $S'$. 
    \begin{itemize}
      \item \bold{Last to Write Rule}
      \item Need to know which transaction is the last to write to the data item.
    \end{itemize}
\end{enumerate}

View  equivalence is also based purely on reads and writes alone. A schedule $S$ is \define{view serializable} if it is view equivalent to a serial schedule. Every conflict serializable schedule is also view serializable. 

A \define{precedence graph} is a direct graph where the vertices are the transactions. We draw an arc from $T_i$ to $T_j$ if the two transactions conflict, and $T_i$ accessed the data item on which the conflict arose earlier. We may label the arc by the item that was accessed. \\ 

A schedule is conflict serializable if and only if its precedence graph is acyclic. Cycle-detection algorithms exist which take order $n^2$ time, where $n$ is the number of vertices in the graph. If precedence graph is acycle, the serializability order can be obtained by a \emph{toplogical sorting} of the graph. The precedence graph test for conflict serializability cannot be used directly to test for view serializability. \\ 

A schedule is \define{recoverable} if a transaction $T_j$ reads a data item previously written by a transaction $T_i$, then the commit operation of $T_i$ appears before the commit operation of $T_j$. \define{Cascading rollback} occurs when a single transaction failure leads to a series of transaction rollbacks; this can lead to the undoing of a significant amount of work. \define{Cascadeless schedules} are those where cascading rollbacks cannot occur: for each pair of transactions $T_i$ and $T_j$ such that $T_j$ reads a data item previously written by $T_i$, the commit operation of $T_i$ appears before the read operation of $T_j$. Every cascadeless schedule is also recoverable. It is desirable to restrict the schedules to those that are cascadeless. 

\section{Concurrency Control}

A \define{lock} is a mechanism to control concurrent access to a data item. Data items can be locked in two modes: 
\begin{enumerate}
  \item \define{exclusive} ($X$) mode. Data item can be both read as well as written. X-lock is requested using \bold{lock-X} instruction. 
  \item \define{shared} ($S$) mode. Data item can only be read. S-lock is requested using \bold{lock-S} instruction.
\end{enumerate}

Lock requests are made to concurrency-control manager. Transaction can proceed only after request is granted. A transaction may be granted a lock on an item if the requested lock is compatible with locks already held on the item by other transactions. Any number of transactions can hold shared locks on an item, but if any transaction holds an exclusive onthe item, no other transaction may hold any lock on the item. If a lock cannot be granted, the requesting transaction is made to wait till all incompatible locks held by other transactions have been released; the lock is then granted. \\ 

A \define{locking protocol} is a set of rules followed by all transactions while requesting and releasing locks. Locking protocls restrict the set of possible schedules. \\ 

The potential for deadlock exists in most locking protocolrs. \define{Starvation} is also possible if concurrency control manager is badly designed. For example, a transaction may be waiting for an exclusive lock on an item, while a sequence of other transactions request and are granted a share lock on the same item; the same transaction is repeatedly rolled back due to deadlocks. Concurrency control manager can be designed to prevent starvation. 

\begin{remark}
  The pitfalls of lock-based protocols are deadlocks and starvation. 
\end{remark}

\define{Two-phase locking protocol} is a protocol which ensures conflict-serializable schedules. 
\begin{enumerate}
  \item \bold{Growing phase}
  \begin{itemize}
    \item transaction may obtain locks 
    \item transaction may not release locks
  \end{itemize}
  \item \bold{Shrinking phase}
  \begin{itemize}
    \item transaction may release locks 
    \item transaction may not obtain locks
  \end{itemize}
\end{enumerate}
The protocol assures serializability. It can be proved that transactions can be serialized in the order of their \bold{lock points} (i.e. the point where a trnasaction acquired its final lock). Two-phase locking does not ensure freedom from deadlocks. Cascading roll-back is possible under two-phase locking. To avoid this, follow a modified protocol called \bold{strict two-phase locking}. Here a transaction must hold all its exclusive locks till it commits/aborts. \bold{Rigoroous two-phase locking} is even stricter: here all locks are held till commit/abort. In this protocol, transactions can be serialized in the order in which they commit. \\ 

Two-phase locking with lock conversions works as follows: 
\begin{enumerate}
  \item \bold{First phase} 
  \begin{itemize}
    \item can acquire a lock-S on item 
    \item can acquire a lock-X on item 
    \item can convert a lock-S to a lock-X
  \end{itemize}
  \item \bold{Second phase}
  \begin{itemize}
    \item can release a lock-S 
    \item can release a lock-X 
    \item can convert a lock-X to a lock-S
  \end{itemize}
\end{enumerate}
This protocol assures serializability, but still relies on the programmer to insert the various locking instructions. \\ 

A \define{lock manager} can be implemented as a separate process to which transactions send lock and unlock requests. The lock manager replies to a lock request by sending a lock grant messages. The requesting transaction awaits until its request is answered. The lock manager maintains a data-strucutre called a \bold{lock table} to record granted locks and pending requests. The lock table is usually implemented as an in-memory hash table and indexed on the name of the data item being locked. \\ 

A system is deadlocked if there is a set of transactions such that every transaction in the set is waiting for another transaction in the set. \define{Deadlock prevention} protocols ensure that the system will never enter into a deadlock state. The following are some prevention strategies.
\begin{itemize}
  \item \bold{Predeclaration}: require that each transaction locks all its data items before it begins execution 
  \item \bold{Graph-based protocol}: impose partial ordering of all data items and require that a transaction can lock data items only in the order specified by the partial order
  \item \bold{Wait-die scheme---non-preemptive}: older transactions may wait for younger one to release data item. Younger transactions never wait for older ones; they are rolled back instead. A transaction may die several times before acquiring needed data. 
  \item \bold{Wound-wait scheme---preemptive}: older transaction wounds (forces rollback) of younger transaction instead of waiting for it. Younger transactions may wait for older ones; may be fewer rollbacks than wait-die scheme.
  \item \bold{Timeout-based schemes}: a transaction waits for a lock only for a specified amount of time. After that, the wait times out and the transaction is rolled back; thus, deadlocks are not possible. 
\end{itemize}

Deadlocks can be described as a \emph{wait-for} graph. The system is in a deadlock state if and only if the wait-for graphy has a cycle. Must invoke a deadlock-detection algorithm periodically to look for cycles. When a deadlock is detected, some transaction will have to be rolled back (made a victim) to break deadlock; select the transaction as victim that will incur minimum cost. Starvation happens if the same transaction is always chosen as victim. Include the number of rollbacks in the cost factor to avoid starvation. 

\section{Recovery System}

\define{Physical blocks} are those blocks residing on the disk. \define{Buffer blocks} are the blocks residing temporarily in main memory. Each transition has its private work-area in which local copies of all data items accessed and updated are kept. \\ 

A \bold{log} is kept on stable storage. The \define{log} is a sequence of log records, and maintains a record of update activities on the database. When a transaction starts, it registers itself by writing a log record. Before the transaction executes, a log record is written where $V$ is the value $X$ before the write, and $V_2$ is the value to be written to $X$. Log record notes that $T_i$ has performed a write on data item $X_j$ which had value $V_1$ before the write, and will have value $V_2$ after the write. When $T_i$ finishes its last statement, the commit statement is written to the log record. We assume, then, that the log records are written directly to stable storage. \\ 

The \define{deferred database modification} scheme records all modificatins to the log, but defers all the writes to after partial commit. Assume that transactions execute serially. A transaction starts by writing \emph{start} to the log. A write operation results in a log record being written with the new value $V$ for $X$. The write is not performed on $X$ at this time, but is deferred. When the transaction partially commits, \emph{commit} is written to the log. Finally, the log records are read and used to actually execute the previously deferred writes. During recovery after a crash, a transaction needs to be redone if and only if both \emph{start} and \emph{commit} are in the log. Redoing a transaction sets the value of all data items updated by hte transaction to the new values. \\ 

The \define{immediate database modification} scheeme allows database updates of an uncommitted transaction to be made as the writes are issued; since undoing may be needed, update logs must have both old value and new value. Update log record must be written before database item is written. We assume that the log record is output directly to stable storage. Also can be extended to postpone log record output, so long as prior to execution of \emph{output} operation for a data block $B$, all log records corresponding to items $B$ must be flushed to stable storage. Output of updated blocks can take place at any time before or after transaction commit. The order in which blocks are output can be different from the order in which they are written. The recovery procedure for immmediate database modification has two operations: 
\begin{itemize}
  \item \emph{undo} restores the value of all data items updated by a transaction to their old values, going backwards from the last log record for the transaction 
  \item \emph{redo} sets the value of all data items updated by a transaction to the new values going forward from the first log record for the transactio 
\end{itemize}
Both operations must be \define{idempotent}: that is, even if the operation is executed multiple times, the effect is the same as if it executed once. When recovering after failure 
\begin{itemize}
  \item A transaction needs to be undone if the log contains the record \emph{start}, but does not contain the record \emph{commit}. 
  \item A transaction needs to be redone if the log contains both the record \emph{start} and the record \emph{commit}.
\end{itemize}
Undo operations are performed first, then redo operations. \\ 

Some problems in recovery procedure include the following 
\begin{enumerate}
  \item Searching the entire log is time-consuming 
  \item We might unnessarily redo transactions which have already output their updates to the database. 
\end{enumerate}
We can streamline the recovery procedure by periodically performing \define{checkpointing}: 
\begin{enumerate}
  \item Output all log recrds currently residing in main memory onto stable storage. 
  \item Output all modified buffer blocks to the disk. 
  \item Write a log record \emph{checkpoint} onto stable storage. 
\end{enumerate}
During recovery we need to consider only hte most recent transaction that started before the checkpoints and transactions that started after that most recent transactions. 
\begin{enumerate}
  \item Scan backwards from end of log to find the most recent checkpoint record\item Continue scanning backwards until a record \emph{start} is found 
  \item Need only consider the part of log following above \emph{start} record. Earlier part of log can be ignored during recovery, and can be erased whenever desired. 
  \item For all transactions with no \emph{commit} execute undo. 
  \item Scanning forward in the log, for all transactions starting from the transaction or later with a \emph{commit}, execute redo. 
\end{enumerate}
