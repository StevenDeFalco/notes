...
Given a set of attributes, $\alpha$, the \define{closure} of $\alpha$ under F (denoted by $\alpha^{+}$) as the set of attributes that are functionally determined by $\alpha$ under $F$. The algorithm to compute $\alpha^{+}$, the closure of $\alpha$ under $F$ is as follows: 
