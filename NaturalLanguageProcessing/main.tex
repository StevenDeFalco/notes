...
\define{Alignment} is the correspondence between particular words in the translated sentence pair. Some words have no counterpart. Alignment can be many-to-one or one-to-many or many-to-many. We learn $P(f, a \vert e)$ as a combination of many factors, including: 
\begin{itemize}
  \item Probability of particular words aligning 
  \item Probability of particular words have particular fertility
\end{itemize}
\define{Neural machine translation} uses a single end-to-end neural network where both the input and output are sentences. Seq2seq models are end-to-end models made up of two RNNs; they are often referred to as encoder-decoder models. The \define{decoder} is a text generator. This is different from a simple text generator because the initial states are determined by the encoder. \\ 

\define{Encoding} of the source sentence provides initial hidden state for the Decoder RNN. The \define{Encoder RNN} produces an encoding of the source sentence. The \define{Decoder RNN} is a language model that generates target sentence, conditioned on encoding. \\ 

The seq2seq model is an example of a \bold{conditional language model}: language model because the decoder is predicting the next word of the target sentence $y$ and conditional because its predictions are also conditinoed on the source sentence $x$. \\ 

In \bold{greedy decoding}, usually we decode until the model produces a <END> token. In \bold{beam search decoding}, different hypotheses may produce <END> tokens on different timestamps; when a hypothesis produces <END> that hypothesis is complete; place it aside and continue exploring other hypotheses via beam search. A small beam size $k$ will be very close to greedy decoding which may result in ungrammatical, unnatural, nonsensical, and incorrect results. A larger $k$ means you consider more hypotheses and thus reduce some of the problems which correspond to a small $k$. \\ 
...
Machine translation can be evaluated using \define{BLEU} (Bilingual Evaluation Understudy) which has n-gram precision plus a penalty for too-short system translations; higher BLEU scores indicate better translation quality. BLEU is useful of imperfect because good translation can still get a poor BLEU score. 

\section{Attention and Transformers}

Attention treats each word's representation as a query to access and incorporate information from a set of values. Attention provides a solution to the bottle neck problem. The core idea of attention is that on each step of the decoder, use direct connection to the encoder to focus on a particular part of the source sequence. \\ 

Use the attention distribution to take a weighted sum of the encoder hidden states. The attention output mostly contains information from the hidden states that received high attention. Then concatenate the attention output with decoder hidden state, then use to compute $\hat{y}$ as before. Sometimes we take the attention output from the previous step, and also feed it into the decoder. \\ 

Writing \define{attention} in equations, we have encoder hidden states $h_1 , \dots , h_N \in \mathbb{R}^{h}$. On timestep $t$, we have decoder hidden state $s_t \in \mathbb{R}^h$. We get the attention scores $e^t$ for this step: $$e^t = [s_t^T h_q , \dots s_t^T h_N] \in \mathbb{R}^N$$ We take softmax to get the attention distribution $\alpha^t$ for this step $$\alpha^t = \textrm{softmax}(e^t) \in \mathbb{R}^N$$ We use $\alpha^t$ to take a weighted sum of the encoder hidden states to get the attnetion outpu $a_t$ $$a_t = \sum_{i=1}^N \alpha_i^t h_i \in \mathbb{R}^h$$ Finally, we concatenate the attnetion output $a_t$ with the decoder hidden state $s_t$ and proceed as in the non-attention seq2seq model $$[a_t ; s_t] \in \mathbb{R}^{2h}$$ Attention significantly improves NMT performance; decoder focuses on certain parts of the source. Attention solves the bottleneck problem; decoder looks directly at source; bypass bottleneck. Attention helps with vanishing gradient problem; provides shortcut to far away states. Attention provides some interpretability. 

\begin{definition}
  Given a set of vector values, and a vector query, attention is a technique to compute a weighted sum fo the values, dependent on the query. 
\end{definition}

There are several ways you can compute $e \in \mathbb{R}^N$ from $h_1 , \dots , h_N \in \mathbb{R}^{d_1}$ and $s \in \mathbb{R}^{d_2}$. 
\begin{itemize}
  \item Basic dot-product attention: $e_i = s^T h_i \in \mathbb{R}$ 
  \item Multiplicative attention: $e_i = s^T W h_i \in \mathbb{R}$ where $W \in \mathbb{R}^{d_2 \times d_1}$ is a weight matrix
  \item Additive attention: $e_i = v^T \textrm{tanh}(W_1 h_i + W_2 s) \in \mathbb{R}$ where $W_1 \in \mathbb{R}^{d_4 \times d_1}$, $W_2 \in \mathbb{R}^{d_3 \times d_2}$ are weight matrices and $v \in \mathbb{R}^{d_3}$ is a weight vector. 
\end{itemize}

\subsection{Self-Attention}

Every word is both a key and a query simultaneously. Let $w_{i:n}$ be a sequence of words in vocabulary $V$. For each $w_i$, let $x_i = Ew_i$, where $E \in \mathbb{R}^{d \times \vert V \vert}$ is an embedding matrix. 
\begin{enumerate}
  \item Transform each word embedding with weight matrices. $$q_i = W^Q x_i$$ $$k_i = W^K x_i$$ $$v_i = W^V x_i$$ 
  \item Compute pairwise similarities between keys and queries; normalize with softamx $$e_{ij} = \textrm{inner\_product}(q_i, k_j) \alpha_{ij} = \frac{\textrm{exp}(eij)}{\sum_{j'}\textrm{exp}(eij')}$$  
  \item Compute output for each word as weighted sum of values $$o_i = \sum_j \alpha_{ij} v_j$$
\end{enumerate}

\section{LLMS: Pretraining and Finetuning}

The key idea is to use a pretrained dataset that is somewhat related to your finetuning data so that the pretrained model will have some good representations for the finetuning task as well. \\ 

\subsection{Pretraining}

In modern NLP, all parameters in NLP networks are initialized via pretraining. Pretraining methods hide parts of the input from the model, and train the model to reconstruct those parts. Pretraining can improve NLP applications by serving as parameter initialization. The first step is to pretrain using lots of data to learn general things, and the second step is to finetune using not many labels with the goal of adapting the model to the task. \\ 

Pretraining establishes the basis of the abilities of LLMs. LLMs can accquire essential language understanding and generation skills. The scale and quality of the pre-training corpus are critical for LLMs to attain powerful capabilities. Model architectures, acceleration methods, and optimization techniques need to be well designed. \\ 

LLMs rely on the pretraining corpus and how it has been preprocessed. Existing LLMs mainly leverage a mixture of diverse public textual datasets as the pre-training corpus. There are two types of pre-training corpora: 
\begin{itemize}
  \item \bold{General data}: with large, diverse, and accessibly nature to enhance the language modeling and generalization abilities of language models 
  \item \bold{Specialized data}: endowing LLMs with specific task-solving capabilities
\end{itemize}

Before pretraining, it is important to remove noisy, redundant, irrelevant, and potentially toxic data. 
\begin{itemize}
  \item Quality filtering 
  \item De-duplication 
  \item Privacy reduction (remove personally identifiable information)
  \item Tokenization 
\end{itemize}

There are three types of architectures that we could pretrain:
\begin{itemize}
  \item \bold{Encoders} get bidirectional context. The idea is to replace some fraction of words in the input with a special mask token; then predict these words. Only add loss terms from words that are masked out. (e.g. BERT)
  \item \bold{Encoder-decoders}. The encoder portion benefits from bidirectional context. The decoder portion is used to train the whole model through language modeling. To pretrain, replace different-length spans from the input with unique placeholders; decode out the spans that were removed. (e.g. BART)
  \item \bold{Decoders}. We can ignore what they were trained to model and we can finetune them by training a classifier on the last word's hidden state. It's natural to pretrain decoders as language models and then use them as generators. (e.g. GPT)
\end{itemize}
...
Pretrained decoders are good at generating sequences. However, BERT and other pretrained encoders don't naturally lead to nice autoregressive (1-word-at-a-time) generation methods. \\ 

BART pre-trains a model combining Bidirectional and Auto-Regressive Transformes. A denoising autoencoder for pretraining sequence-to-sequence models. BART is trained by corrupting text with an arbitrary noising function and learning a model to reconstruct the original text. BART uses a standard transformer-based neural machine translation architecture. \\ 

Very large language models seem to perform some kind of learning without gradient steps simply from examples you provide within their contexts. The \define{in-context} examples seem to specify the task to be performed, and the conditional distributin mocks performing the task to a certain extent. There are three approaches to in-context learning 
\begin{itemize}
  \item \define{Few-shot}: includes several examples in the prompt to demonstrate the expected answer format and content. 
  \item \define{One-shot}: only a single example is provided in the prompt to demonstrate the desired task. 
  \item \define{Zero-shot}: no examples are provided to the model during the generation call. Instead, only the task or request is given as input. 
\end{itemize}

\subsection{Finetuning}
 
\define{Full finetuning} means we update all the parameters in the network during the finetuning process. \define{Parameter-efficient finetuning} trains usinga few existing o r new parameters. Paramter-efficient finetuning aims to reduce the number of trainable parameters while retaining good performance. There are four parameter-efficient finetuning methods for transformer language models: 
\begin{enumerate}
  \item \define{Adapter tuning} incorporates small neural network modules (called adapter) into each transformer layer. During finetuning, the adapter modules would be optimized according to the specific task goals, freeze the paramters of the original language. 
  \item \define{Prefix tuning} adds a prefix of parameters, and freezes all pretrained parameters. The prefix is processed by the model just like real words would be. Preprends a sequence of prefixes, which are a set of trainable continuous vectors, to each Transformer layer in language models. 
  \item \define{Prompt tuning} incorporates trainiable prompt vectors at the input layer. Augments the input text by including a group of soft prompt tokens, then takes the prompt-augmented input to solve specific downstream tasks. During training, only the prompt embeddings would be learned according to task-specific supervisions. 
  \item \define{Low-Rank Adaptation (LoRa)} learns low-rank "diff" between the pretrained and finetuned weight matrices. This is easier to learn than prefix-tuning. 
\end{enumerate}

\section{Dependency Parsing}

There are two views of linguistic structure: \bold{constituency} and \bold{dependency structure}. In the \define{consitituency structure}, phrase structure organizes words into nested constitutents; starting units are words in some category, words are combined into phrases, and phrases can be combined into bigger phrases. \define{Dependency structure} shows which words depend on (modify or are arguments of) which other words. \\ 

We need to understand sentence structures in order to be able to interpret language correctly. \define{Prepositional phrase attachment ambiguity} arises in language when a prepositional phrase can be attacned to more than one part of a sentence, resulting in different possible interpretations of the sentence's meaning. A key parsing decision is how we \emph{attach} different various constitutents. \\ 

\define{Coordination scope ambiguity} is a type of syntactic ambiguity that arises in sentences containing coordinate structures. Coordinate structures are sentences or phrases where two or more elements of similar syntactic type are joined together using conjunctions. The ambiguity occurs when it's not clear which elements are being coordinated with each other. \\ 

Dependency syntax assumes that syntactic structure consists of lexical items, normally binary asymmetric relations (arrows) called \define{dependencies}. The arrows are commonly typed with the name of grammatical relations. The arrow connects a head with a dependent. Usually, dependencies form a tree. \\ 

\define{Dependency parsing} is the process to analyze the grammatical structure in a sentence and find out related words as well as the type of the relationship between them. \define{Distributed representations} represent each word as a $d$-dimensional dense vector; similar words are expected to have close vectors. Meanwhile, part-of-speech tags and dependency labels are also represented as $d$-dimensional vectors; the smaller discrete sets may also exhibit many semantical similarities. \\ 
