...
A \define{European call} option is a contract giving the holder the right to buy an asset for price $X$ fixed in advance at a specified future time $T$. 
\begin{itemize}
  \item The asset is called the underlying 
  \item The fixed price, $X$, is known as the exercise price 
  \item The time $T$ is knwon as the exercise or expiry time
\end{itemize}
Similar, to European Call, a European put option gives the right to sell the underlying asset for the strike price $X$ at the exercise time $T$. \\ 

An \define{American call} (respectively put) option gives the right to buy (respectively sell) the underlying asset for the strike price $X$ at any time between now and a specified future time $T$, called the expiry time. In other words, an American option can be exercised at any time up to and including expiry.  

\begin{remark}
  The price of an American option will be greater than that of a European option because it allows for more flexibility
\end{remark}

The option price is deteremined by its payoff: the higher the payoff, the higher the price (vice versa). For a European call, the payoff is $$S(T) - X$$ if $S(T) > X$, or else it is simply 0. 

\begin{remark}
  The notation $x^{+}$ is used to denote the max between 0 and $x$
\end{remark}

Since payoffs are non-negative, a premium must be paid to buy an option. If no premium had to be paid, an investor purchasing an optin could under circumstances lose money and would in fact make a profit whenever the payoff turned out to be positive. This would be contrary to the No-Arbitrage Principle. \\ 

Establishing bounds and some general properties for option prices is the key to price options. The gain of an option buyer is the payoff modified by the premium $C_E$ or $P_E$ paid for the option. At time $T$ the gain of the buyer of a European call is $$(S(T) - X)^+ - C_E e^{rT}$$ which takes into account the time value of the premium. For the buyer of a European put the gain is $$(X - S(T))^+ - P_E e^{rT}$$ Consider a portfolio constructed by writing and selling one put and buying one call option: both have the same strike price $X$ and the same exercise date $T$. Adding the payoffs of the long position in calls and the short position in puts, we obtain the payoff of a \bold{long forward contract} with forward price $X$ and delivery time $T$.\\ 

For a stock that pays no dividends the following relation holds between the prices of European call and put options, both with exercise price $X$ adn exercise time $T$ (\define{Put-Call Parity}) $$C_E - P_E = S(0) - Xe^{-rT}$$ If the asset pays dividend, the option only gives me the right to acquire after the fact, then the price of the options should reflect that too. For a stock paying a single dividend $D$ in the future at time $t$, where $0 < t < T$, the call-put parity indicates that $$C_E - P_E = S(0) - De^{-rT} - Xe^{-r}$$ The prices of American put and call options with the same strike price $X$ and expiry time $T$ on a stock that pays no dividends satisfy $$S(0) - Xe^{-rT} \geq C_A - P_A > S(0) - X$$ We say that at time $t$ a \define{call option} with strike price $X$ is 
\begin{itemize}
  \item In the money (ITM) if $S(t) > X$ 
  \item At the money (ATM) if $S(t) = X$ 
  \item Out of the money (OTM) if $S(t) < x$
\end{itemize}
We say that at time $t$ a \define{put option} with strike price $X$ is 
\begin{itemize}
  \item In the money (ITM) if $S(t) < X$ 
  \item At the money (ATM) if $S(t) = X$ 
  \item Out of the money (OTM) if $S(t) > x$
\end{itemize}
At time $t \leq T$, the \define{instrinsic value} of a call option with strike price $X$ is equal to $(S(t) - X)^+$. The \define{intrinsic value} of a put option with the same strike price is $(X - S(t))^+$. The \define{time value} of an option is the difference between the price of the option and its intrinsic value. 

\section{Binomial Model and Option Pricing}

Suppose that for any contingent claim $D(T)$ there exists a replication strategy. Specifically, an admissible strategy $x(t), y(t)$ with final value $V(T) = D(T)$. The price $D(0)$ of the contingent claim at time 0 must be equal to that of the replicating strategy, $V(0) = D(0)$. \\ 

The contingent claim at time 1 has two potential realizations: $f(S(1)) = f(S^u)$ with proabability $p$ and $f(S(1)) = f(s^d)$ with probability $1-p$. To replicate a general derivative security with payoff $f$, we hold $x$ stocks and $y$ bonds. It follows that $$xS^u = y(1+r) = f(S^u)$$ $$xS^d = y(1+r) = f(S^d)$$ We can derive the values of $x$ and $y$ given the above two equations $$x = \frac{f(S^u) - f(S^d)}{S^u - S^d}$$ $$y = \frac{(1+d)f(S^u) - (1+u)f(S^d)}{(u-d)(1+r)}$$ While the future value of stock can never be known with certainty, it is possible to work out expected stock prices within the binomial tree model. Since $f$ is a contingent claim on $S(1)$, then the realization of $f$ also depends on the probability of $S(1)$ $$\mathbb{E}[f(S(1))] = p \times f(S^u) + (1-p) \times f(S^d)$$ The contingent claim price should reflect future discounted cash flows. In this regard, one expects that the following relationship holds true $$D(0) = \frac{1}{1+r} \mathbb{E}[f(S(1))]$$ The \define{risk-neutral probability} is a probability measure $p^{*} \in (0,1)$ under which it holds true that $$\mathbb{E}^{\mathbb{Q}}[S(1) \vert S(0)] = S(0)(1+r)$$ or alternatively $$\mathbb{E}^{\mathbb{Q}}[R_S] = r$$ is called a risk-neutral probability. 

\begin{remark}
  It is common to differentiate the risk-neutral from the physical probability using the superscript $\mathbb{Q}$ 
\end{remark}

According to the one-step binomial model, we know that $$p^{*} = \frac{r-d}{u-d}$$ Hence, the fair price (no-arbitrage) price of the contingent claim is $$D(0) = \frac{1}{1+r} \mathbb{E}^{\mathbb{Q}}[f(S(1))] = \frac{1}{1+r}[p^* f(S^u) + (1-p^*)f(S^d)]$$ For any $n=0,1,2, \dots$, the \define{martingale property} states that $$\mathbb{E}^{\mathbb{Q}}[\tilde{S}(n+1) \vert S(n)] = \tilde{S}(n)$$ The risk neutral probability, $p^*$, is also referred to as the \emph{martingale probability}. Since the pricce of the European option represents the expectation of the discounted payoff computed with respect to the risk-neutral probability, then we know that $$D(0) = \frac{1}{(1+r)^2} \mathbb{E}^{\mathbb{Q}} [f(S(2))]$$ 
